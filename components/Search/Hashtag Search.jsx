...
export default function HashtagSearch({ query }) {
    let a = 0;
    const { blogs, loading } = UseFetch(`${fakeLinks.hashsearch}${query}`)
    return (
        <>
            {blogs.length !== 0 && <h3 style={{ marginTop: `1em` }}>Search Results for {query} in Hashtags</h3>}
            {blogs.length !== 0 && <div className={PostStyle.row}>
                {!loading && blogs.map((blog) => {

                    return <ClientSideCards key={`hashtagSearchBlog${blog.id}`} slug={blog.slug} PostContent={blog.Content} image={blog.Image} PostHead={blog.Headline} PostName={blog.Headline} date={blog.date} time={blog.Time} />
                })}
